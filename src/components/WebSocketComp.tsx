...
/**
 * @file WebSocketComp.tsx
 * @description 웹소켓 통신을 통해 실시간 크롤링 진행 상황을 표시하고,
 *              테스트 푸시 알림을 전송하는 기능을 제공하는 React 함수형 컴포넌트입니다.
 *              `useWebSocket` 커스텀 훅을 사용하여 웹소켓 연결 로직을 분리하고,
 *              `useUserStore`를 통해 전역 userId를 가져와 사용합니다.
 *
 * - `useWebSocket` 훅을 통해 백엔드 웹소켓 서버와 연결하여 크롤링 데이터를 실시간으로 수신합니다.
...
const WebSocketComp: React.FC = () => {
    // 전역 Zustand 스토어에서 userId를 가져옵니다.
    ...
    // useWebSocket 훅의 메시지 핸들러를 정의합니다.
    ...
    // useWebSocket 훅을 사용하여 웹소켓 연결을 관리합니다.
    // wsUrl은 훅 내부에서 userId를 사용하여 구성됩니다.
