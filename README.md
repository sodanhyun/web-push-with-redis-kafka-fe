...
4.  **크롤링 시작**: "크롤링 시작" 버튼을 누르면 백엔드에서 크롤링 프로세스가 시작되며, WebSocket을 통해 실시간 진행 상황(진행률 및 상태)을 확인할 수 있습니다.
...
#### `src/components/`
*   **`WebSocketTest.tsx`**: 실시간 WebSocket 통신을 시연하는 컴포넌트입니다. 백엔드로부터 `CrawlingProgressMessageDto` 형태의 크롤링 진행 데이터를 받아 진행률과 상태를 화면에 표시합니다.
